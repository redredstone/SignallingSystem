				...
				if (action.toLowerCase().equals("add")) {
					if (args.length < 2) {
						...
						if (item.toLowerCase().equals("signal")) {
							...
							Location center = origin.clone().add(dir).add(0,plrLoc + 1,0);
							...
							//Building a signal
							Location blockLocation = center.clone().add(rot).add(0,-95,0);
							...
							//Item frames
							Entity frame1 = (Entity) plr.getWorld().spawnEntity(blockLocation.add(dir), EntityType.ITEM_FRAME);
							Entity frame2 = (Entity) plr.getWorld().spawnEntity(blockLocation.add(0,-1,0), EntityType.ITEM_FRAME);
